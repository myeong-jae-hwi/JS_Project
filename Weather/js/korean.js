...
function weatherIcon(w_id){

  var rain_code = [200, 201, 232, 300, 310, 313, 314, 321, 500, 501, 503, 504, 520, 521, 522, 531];
  var snow_code = [600, 601, 602, 611, 612, 615, 616, 620, 621, 622]
  var sunny_code = [800, 801, 802, 803]
  var cloud_code = [202, 210, 211, 212, 221, 230, 231, 721, 804, 951, 952, 953, 955]
  var wind_code = [771, 900, 901, 902, 905, 954, 956, 957, 958, 959, 961, 962]
  var foggy_code = [701,741, 731, 751, 761, 762]

  if(sunny_code.includes(w_id)){
    sunny_background();
    return('image/sunny_icon.png');
  }

  else if(cloud_code.includes(w_id)){
    cloud_background();
    return('image/cloudy_icon.png');
  }

  else if(snow_code.includes(w_id)){
    fallingsnow();
    snowy_background();
    return('image/snow_icon.png');
  }

  else if(foggy_code.includes(w_id)){
    foggy_background();
    return('image/foggy_icon.png');
  }

  else if(wind_code.includes(w_id)){
    // foggy_background();
    return('image/wind_icon.png');
  }

  else if(rain_code.includes(w_id)){
    rain();
    rainy_background();
    return('image/rain_icon.png');
  }
}